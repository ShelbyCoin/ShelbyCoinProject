...
#if defined(NDEBUG)
# error "Shelbycoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xc80feedcc8b4553bd58d560f7ba7df4eac2263c155bf3eed23b7a1d2ea27bf8f");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Shelbycoin: starting difficulty is 1 / 2^12
...
{
    // Shelbycoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 12000); // Shelbycoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1.5 * 24 * 60 * 60; // Shelbycoin: 3.5 days
static const int64 nTargetSpacing = 4 * 60; // Shelbycoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Shelbycoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Shelbycoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x378ee10fb377a3cb4aec54a7261c7029242937ab828913555e1dc69a5100a623");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "First legal race betting contract";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 30 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04a7baa11b7e4d5554b7ff46c93b26de0281282d1341c7ee668acbf3d7a0fd421517846349d46f82b7c573c78b0283f86b84616bfefe1ec9611f4968607fa89fb2") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1546720395;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084525862;

        if (fTestNet)
        {
            block.nTime    = 1546720380;
            block.nNonce   = 385498800;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x6ccc43bf6ac4032d77765b357d15228f9a0b6e98a3acf0d124c00724c6599bca"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc4, 0xbd, 0xd2 }; // Shelbycoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ShelbycoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ShelbycoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ShelbycoinMiner(CWallet *pwallet)
{
    printf("ShelbycoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running ShelbycoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ShelbycoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ShelbycoinMiner, pwallet));
}
